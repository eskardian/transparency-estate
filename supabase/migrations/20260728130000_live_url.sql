-- Прямой эфир: источник-независимая ссылка на трансляцию у объекта.
-- YouTube Live (путь 1) или свой HLS-поток (путь 2) — портал сам выберет плеер. Идемпотентна.
alter table objects add column if not exists live_url text;
