-- Документы для клиента: флаг client_visible; портал показывает такие документы.
-- Скачивание — через Edge Function sign-doc (подписывает ссылку по share-токену). Идемпотентна.
alter table object_documents add column if not exists client_visible boolean not null default false;

-- RPC портала: + список видимых клиенту документов (id, name). Ссылку подпишет Edge Function.
