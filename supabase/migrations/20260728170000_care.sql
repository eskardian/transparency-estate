-- Сезонный уход: флаг «объект на обслуживании» (обычно после сдачи). Идемпотентна.
alter table objects add column if not exists care boolean not null default false;

-- RPC портала: + care (клиент видит бейдж «на сезонном обслуживании»).
